...
client.on('message', async message => {
	...
	if (command === 'balance' || command === 'bal') {
		...
		return message.channel.send(`Successfully transferred ${transferAmount}:apple: to ${transferTarget.tag}. Your current balance is ${currency.getBalance(message.author.id)}:apple:`);
		...
		return message.channel.send(items.map(i => `${i.name}: ${i.cost}:apple:`).join('\n'), { code: true });
		...
		return message.channel.send(
			currency.sort((a, b) => b.balance - a.balance)
				...
				.first(10)
				.map((user, position) => `(${position + 1}) ${(client.users.get(user.user_id).tag)}: ${user.balance}:apple:`)
				.join('\n'),
			{ code: true }
		);
