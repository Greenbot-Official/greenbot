...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    let wtext = ''
    let etext = ''
    if (items.filter(a => a.type == 'c' && a.amount > 0).map(item => `${item.item_id}`).join('\n')) ctext = 'consumables:\n' + await items.sort((a, b) => a.id - b.id).filter(a => a.type === 'c' && a.amount > 0).map(item => `[${item.shop_id}]${item.amount} ${item.item_id} heal:${item.heal}`).join('\n')
    if (items.filter(a => a.type == 'w' && a.amount > 0).map(item => `${item.item_id}`).join('\n')) wtext = `${ctext ? '\n\n' : ''}weapons:\n` + await items.sort((a, b) => a.id - b.id).filter(a => a.type === 'w' && a.amount > 0).map(item => `[${item.shop_id}]${item.amount} ${item.item_id} damage:${item.damage} ${item.attribute != 'none' ? `attribute: ${ item.attribute }` : ''} ${!item.enchant ? '' : `enchant:${item.enchant}`}`).join('\n')
    if (items.filter(a => a.type == 'e' && a.amount > 0).map(item => `${item.item_id}`).join('\n')) etext = `${wtext || ctext ? '\n\n' : ''}enchantments:\n` + await items.sort((a, b) => a.id - b.id).filter(a => a.type === 'e' && a.amount > 0).map(item => `[${item.shop_id}]${item.amount} ${item.item_id}  enchant cost:${item.ecost}`).join('\n')
